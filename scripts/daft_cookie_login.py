#!/usr/bin/env python3
"""
Daft.ie Cookie Login Script

Opens Chrome locally (visible window), navigates to Daft.ie,
...
Usage:
    ...
    python scripts/daft_cookie_login.py

    # Or specify Redis host if Docker Redis is on a different host:
    REDIS_HOST=localhost REDIS_PORT=6379 python scripts/daft_cookie_login.py

After running:
    1. Chrome opens → Daft.ie loads
...
def main():
    ...
    # ── Config ────────────────────────────────────────────────────────────────
    DAFT_URL = "https://www.daft.ie/property-for-rent/ireland"
    ...
    print("=" * 60)
    print("  Daft.ie Cookie Login — Cloudflare Bypass")
    print("=" * 60)
    print()
    ...
    # Must match STEALTH_USER_AGENT in services/collector/uc_driver.py
    ...
    # ── Navigate ──────────────────────────────────────────────────────────────
    print(f"[2/4] Navigating to {DAFT_URL}")
    print("      Solve the Cloudflare challenge in the browser window...")
    print()
    driver.get(DAFT_URL)

    # ── Wait for user ─────────────────────────────────────────────────────────
    # Poll until the page title changes from "Just a moment..." or user presses ENTER
    ...
    try:
        ...
        fallback_path = os.path.join(os.path.dirname(__file__), "daft_cookies.json")
